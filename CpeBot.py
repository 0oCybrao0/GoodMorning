...
class CPEBot:
    ...
    def login(self):
        ...
        while True:
            try:
                resCaptcha = self.session.get(f"https://cpe.cse.nsysu.edu.tw/cpe/{captcha}", timeout=600)
        ...
        if res.is_redirect:  # login success
            while True:
                try:
                    res = self.session.get("https://cpe.cse.nsysu.edu.tw/cpe/newest", timeout=600)
            ...
            self.user_data["mySchoolID"] = soup.select_one("select[name='mySchoolID'] option[selected]").get("value")
...
if __name__ == "__main__":
    ...
    bot = CPEBot("B123620637", "CreeperB0813")
    bot.login()
    bot.apply(33)