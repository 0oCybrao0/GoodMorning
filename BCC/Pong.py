...
def reset_ball(side):
    ...
    if ball_y >= 240:
        ball_speed_y = random.uniform(-3, -8)
    elif ball_y < 240:
        ball_speed_y = random.uniform(3, 8)

    if side == "right":
        ball_speed_x = 4 + (score1 + score2) * 0.4
        score1 += 1
        if score1 == 11:
            pygame.quit()
        ball_x = 80
...
while running:
    ...
    paddle1_rect = pygame.Rect(paddle1_x, paddle1_y, paddle_width,
                               paddle_height)
    paddle2_rect = pygame.Rect(paddle2_x, paddle2_y, paddle_width,
                               paddle_height)
    # see if the rectangles overlap
    if doRectsOverlap(ball_rect, paddle1_rect):
        ball_speed_x = -ball_speed_x * speed_boost
        paddle_color_1 = random_color()
        ball_color = random_color()
    elif doRectsOverlap(ball_rect, paddle2_rect):
        ball_speed_x = -ball_speed_x * speed_boost
        paddle_color_2 = random_color()
        ball_color = random_color()

    # draw everything on the screen
    score1_label = myfont.render(str(score1), 1,
                                 pygame.color.THECOLORS['white'])
    screen.blit(score1_label, (5, 10))
    score2_label = myfont.render(str(score2), 1,
                                 pygame.color.THECOLORS['white'])
    screen.blit(score2_label, (620, 10))
    pygame.draw.circle(screen, ball_color,
                       [int(ball_x), int(ball_y)], ball_radius, 0)
    pygame.draw.rect(screen, paddle_color_1,
                     [paddle1_x, paddle1_y, paddle_width, paddle_height], 0)
    pygame.draw.rect(screen, paddle_color_2,
                     [paddle2_x, paddle2_y, paddle_width, paddle_height], 0)
